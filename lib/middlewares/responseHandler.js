...
function responseHandler() {
  return async (ctx, next) => {
    ...
    ctx.res.fail = ({
      ...
      statusCode: 400, code: null, error: null, message: null,
    }) => {
      const status = 'fail';
    ...
    ctx.res.error = ({
      statusCode, code, error = null, message = null,
    }) => {
      const status = 'error';
    ...
    ctx.res.missingParameter = (params = {}) => {
      ctx.res.fail({
        ...params,
    ...
    ctx.res.invalidArgument = (params = {}) => {
      ctx.res.fail({
        ...params,
        statusCode: statusCodes.CONFLICT,
      });
    ...
    ctx.res.sequelizeIfError = (params = {}) => {
      const { code = null, errors = null, message = null } = params;

      const error = {
        errors,
      };

      ctx.res.argumentError({
        code,
        error,
        message,
      });
